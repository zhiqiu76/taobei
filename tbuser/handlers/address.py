...
def update_address(id):
	...
	if data.get('is_default'):
		Address.query.filter(Address.is_default == True).update({'is_default':False})
...
def address_infos():
	...
	addresses = {address.id: AddressSchema().dump(address) for address in query}
	return json_response(address=addresses)